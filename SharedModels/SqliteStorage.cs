...
CREATE TABLE IF NOT EXISTS InvertedIndex (
    ...
    PRIMARY KEY (Word, CanonicalUrl),
    FOREIGN KEY (CanonicalUrl) REFERENCES Pages(CanonicalUrl) ON DELETE CASCADE
);

CREATE INDEX IF NOT EXISTS IX_InvertedIndex_Word ON InvertedIndex(Word);
...
ON CONFLICT(CanonicalUrl) DO UPDATE SET
    ...
    CrawlDepth = MIN(Pages.CrawlDepth, excluded.CrawlDepth),
...
VALUES (@u,@ts,@t,@md,@mk,@b,@hj,@lj,@ch)";
        ...
        public async Task UpsertInvertedIndexAsync(string word, string url, int freq)
{
    using var conn = new SQLiteConnection(_connStr);
    await conn.OpenAsync();
    ...
    using var cmd = conn.CreateCommand();
    cmd.CommandText = @"
        INSERT INTO InvertedIndex (Word, CanonicalUrl, Frequency)
        VALUES ($w, $u, $f)
        ON CONFLICT(Word, CanonicalUrl)
        DO UPDATE SET Frequency = $f;
    ";
